...
Le Ryzen 9 5950X est un processeur haut de gamme avec d'excellentes performances single-thread et multi-thread, ce qui en fait une référence exigeante pour la comparaison CPU/GPU.
...
\textbf{Latence} & CL16-18-18 (estimé) \\
\hline
\end{tabular}
\end{center}
...
La RTX 5060 est une carte de dernière génération avec l'architecture Blackwell, offrant un excellent rapport performance/prix pour le calcul GPGPU.
...
\hline
v1.0 $\rightarrow$ v1.1 & Hystérésis GPU & \textbf{+346\%} & Élimination transferts CPU$\leftrightarrow$GPU \\
v1.1 $\rightarrow$ v1.2 & Sans cudaDeviceSync & $\approx$ 0\% & Déjà pipeliné par le driver \\
v1.2 $\rightarrow$ v1.3 & Shared memory & $\approx$ 0\% & Rayon trop petit (3px) \\
v1.3 $\rightarrow$ v1.4 & Fusion kernels & $\approx$ 0\% & Cache L2 déjà efficace \\
\hline
\end{tabular}
\end{center}
